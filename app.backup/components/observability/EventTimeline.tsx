...
/**
 * EventTimeline - Real-time event feed component
 *
 * Inspired by LiveKit ChatTile auto-scroll pattern.
 * Displays observability events in chronological order with auto-scroll.
 *
 * Features:
 * - Auto-scroll to latest events (chat-style UX)
    ...
    <div className="flex flex-col gap-2 p-4 rounded-lg bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <span
            className={cn(
              ...
              eventTypeColors[event.eventType] || "bg-gray-500/10 text-gray-600"
            )}
          >
            {event.eventType}
          </span>
          ...
          {event.llmProvider && (
            <span
              className={cn(
                "px-2 py-1 text-xs font-medium rounded-full whitespace-nowrap",
                llmProviderColors[event.llmProvider] || "bg-gray-500 text-white"
              )}
            >
              {event.llmProvider}
      ...
      {(event.duration || event.tokensUsed || event.costEstimate) && (
        <div className="flex items-center gap-4 text-xs text-gray-500 dark:text-gray-400">
          ...
          {event.tokensUsed && <span>🔤 {event.tokensUsed.toLocaleString()} tokens</span>}
          {event.costEstimate && (
